(() => {
	...
	newPost.addEventListener('submit', event => {
		...
		let postinput = document.getElementById("new-post-text").value;
		window.tweet.post(postinput);
	});
	...
	firebase.auth().onAuthStateChanged(user => {
		...
		let posts = document.getElementById("profile-posts-count");
		let reactions=document.getElementById("profile-likes-count");
		let user = auth.getUserStats(user.uid);
		user.once('value').then(snapshot => {
			posts.innerHTML = parseInt(snapshot.val()['tweets']);
			reactions.innerHTML = parseInt(snapshot.val()['likes']);
		});
	...
	tweetsDB.on('child_added', data => {
		...
		postContainer.append(new_post);

		let like_button = new_post.querySelector(".like-btn");
		like_button.addEventListener("click", event => {
			event.preventDefault();
			window.tweet.incrementLikes(event.target.getAttribute("data-id"));
		})

		let dislike_button = new_post.querySelector(".dislike-btn");
		dislike_button.addEventListener("click", event => {
			event.preventDefault();
			window.tweet.decrementLikes(event.target.getAttribute("data-id"));
		})

		let delete_button = new_post.querySelector('.post-close');
